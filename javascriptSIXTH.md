...
如果省略数组字面量中的某个值，省略的元素将被赋予`undefined`值

```
var arr1 = [,,3] //数组有3个元素，前俩个为undefined
var arr2 = [,,] //数组有2个元素，都是undefined
```

**数组直接量的语法允许有可选的逗号结尾，故`[,,]`只有2个元素。**	
...
> 数组索引仅仅只是对象属性名的一种特殊类型

```
a[-1.23] = true  //将创建一个属性名为"-1.23"的属性
a['1000'] = 0    //这是数组的第1001个元素
a[1.00]          //和a[1]相等
```

> 由于数组是对象,因此也可以从原型中继承元素，可以使用`getter`和`setter`方法
